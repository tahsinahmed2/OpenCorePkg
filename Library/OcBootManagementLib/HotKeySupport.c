/** @file
  ...
  Additional contributions copyright (C) 2021, Mike Beaton. All rights reserved.
  ...
  )
{
  ...
  KeyMap = OcGetProtocol(
    &gAppleKeyMapAggregatorProtocolGuid,
    DEBUG_ERROR, "OCHK: OcLoadPickerHotKeys missing AppleKeyMapAggregator - %r\n"
  );
  ...
  )
{
  ...
  //
  // Failsafe to false 'do nothing'; but note that even when true, legacy codepath
  // is used as a fallback e.g. if protocol not found or memory cannot be allocated.
  //
  mUseDownkeys = FALSE;
  ...
  if (AsciiStrCmp (DownkeysHandlerStr, "Enabled") == 0) {
    ...
    DEBUG ((DEBUG_WARN, "OCHK: Invalid DownkeysHandler setting - using Disabled\n"));
  }

  if (mUseDownkeys) {
    KeyMap = OcGetProtocol(
      &gAppleKeyMapAggregatorProtocolGuid,
      DEBUG_ERROR, "OCHK: OcInitDownkeys missing AppleKeyMapAggregator - %r\n"
    );
    ...
    //
    Status = OcRegisterTypingHandler(&mTypingContext);
    ...
    DEBUG ((DEBUG_INFO, "OCHK: OcInitDownkeys - %r\n", Status));
  ...
  )
{
  ...
  ASSERT (ARRAY_SIZE (KeysDoNotRepeat) >= ARRAY_SIZE (AkmaKeys)); // for CopyMem in legacy codepath
  ...
  if (mUseDownkeys) {
    ...
    //
    Keys                = &Key;
    OcGetNextKeystroke(mTypingContext, &Modifiers, Keys);
    if (Key == 0) NumKeys = 0;
    else NumKeys = 1;

    DEBUG_CODE_BEGIN();
    OcShowKbDebugDisplay (0, NumKeys, 0, Modifiers, L' ', TRUE); // normal
    DEBUG_CODE_END();
    ...
    // AKMA keys and typing keys are the same; additional flush code is enabled.
    //
    Keys                = AkmaKeys;
    NumKeys             = ARRAY_SIZE (AkmaKeys);
    Status = KeyMap->GetKeyStrokes (
      KeyMap,
      &Modifiers,
      &NumKeys,
    ...
    DEBUG_CODE_BEGIN();
    OcShowKbDebugDisplay (0, 0, NumKeys, Modifiers, L' ', FALSE); // normal
    DEBUG_CODE_END();

    AkmaNumKeys = NumKeys;
    AkmaModifiers = Modifiers;
